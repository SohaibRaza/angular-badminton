...
@Component({
  ...
  templateUrl: './physical-content.component.html',
  styleUrls: ['./physical-content.component.scss'],
})
export class PhysicalContentComponent implements OnInit {
  sections: any;
  ...
  ngOnInit() {
    this.activeRouter.params.subscribe(res => {
      ...
      this.pageService.getPageByParent(res.id, 1, 50).subscribe(async (res: any) => {
        ...
        const cacheLatest = await this.storageService.get('latestPhysical');
